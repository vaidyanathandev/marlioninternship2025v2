...
import { Countdown, StreamCard } from "@marlion/ui";
import { INTERNSHIP_STREAMS } from "@marlion/types";
import { generateFAQResponse } from "@marlion/ai";
...
const Home: React.FC = () => {
  ...
  // Fetch latest announcements from Convex
  const announcements = useQuery(api.support.getAnnouncements);
  ...
  const handleAskAI = async (e: React.FormEvent) => {
    ...
    try {
      const response = await generateFAQResponse(aiQuery);
    ...
    <div className="min-h-screen flex flex-col bg-marlion-bg selection:bg-marlion-primary selection:text-white font-sans">
            ...
            {user ? (
              <>
                ...
                <button
                  ...
                  className="bg-white text-slate-950 hover:bg-slate-200 px-5 py-2.5 rounded-lg text-sm font-bold transition-all shadow-lg hover:shadow-white/10 transform hover:-translate-y-0.5"
                >
                  {role === "STUDENT" ? "Dashboard" : "Complete Registration"}
                </button>
              </>
            ) : (
              <>
                <button
                  ...
                  className="text-slate-400 hover:text-white font-medium text-sm transition-colors px-2"
                >
                  Login
                </button>
                <button
      ...
      <header className="relative pt-36 pb-24 px-6 overflow-hidden">
        ...
        <div className="container mx-auto text-center max-w-6xl relative z-10">
          ...
          <p className="text-lg md:text-xl text-slate-400 max-w-3xl mx-auto mb-12 leading-relaxed font-light">
            ...
            <span className="text-white font-semibold">Assistive Tech & IEP</span> for
            Neurodiverse Children. Master AI, XR, and Full Stack development.
          </p>
          ...
          <div className="flex flex-col items-center gap-12">
            ...
            <div className="flex flex-col sm:flex-row gap-5 w-full justify-center">
              <button
                onClick={handleRegisterClick}
                className="bg-gradient-to-r from-marlion-primary to-blue-600 hover:to-blue-500 text-white px-10 py-4 rounded-xl font-bold text-lg shadow-[0_10px_40px_-10px_rgba(59,130,246,0.5)] transition-all transform hover:-translate-y-1 hover:scale-105 active:scale-95 ring-1 ring-white/20"
              >
                Apply Now
              </button>
              <a
                ...
                className="glass-button text-white px-10 py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-3 group hover:bg-white/5"
              >
                Explore Streams{" "}
                <i className="fa-solid fa-arrow-down group-hover:translate-y-1 transition-transform text-sm"></i>
              </a>
            </div>
          </div>

          {/* CEO Message Video Placeholder */}
          <div
            className="mt-24 max-w-5xl mx-auto relative group cursor-pointer"
            onClick={() =>
              showToast(
                "CEO video will be embedded here with actual YouTube URL",
                "info"
              )
            }
          >
            ...
            <div className="relative aspect-[21/9] bg-slate-900 rounded-2xl border border-slate-800 flex flex-col items-center justify-center overflow-hidden shadow-2xl">
              ...
              <p className="mt-6 text-slate-400 font-medium tracking-widest text-sm uppercase z-10">
      ...
      <section id="streams" className="py-32 px-6 container mx-auto relative">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto items-start">
          {Object.values(INTERNSHIP_STREAMS).map((stream, index) => (
            <StreamCard
              key={stream.id}
              stream={stream.title}
        ...
        <div className="container mx-auto px-6 max-w-4xl text-center relative z-10">
          ...
          >
            ...
            <button
              type="submit"
              disabled={isThinking}
              className="absolute right-2 top-2 bottom-2 w-14 bg-marlion-primary text-white rounded-full flex items-center justify-center hover:bg-marlion-primaryHover transition-all z-10 shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50"
            >
              {isThinking ? (
        ...
        <section className="py-20 px-6 container mx-auto">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
              <i className="fa-solid fa-bullhorn text-marlion-accent"></i>
            ...
            <div className="space-y-4">
              {announcements.slice(0, 3).map((announcement: any) => (
                <div
                  ...
                  className="glass-card p-6 rounded-xl border border-marlion-border hover:border-marlion-primary/30 transition-all"
                >
        ...
        <div className="container mx-auto grid md:grid-cols-2 gap-20 max-w-6xl">
          <div>
            ...
            <div className="space-y-6">
              <p className="text-slate-400 flex items-center gap-4 group cursor-pointer transition-colors hover:text-white">
                <span className="w-10 h-10 rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center text-marlion-primary group-hover:bg-marlion-primary group-hover:text-white transition-all">
          ...
          <div>
            ...
            <p className="text-slate-400 leading-loose mb-8 text-lg">
              A-34, Kumarasamy Street,
              <br />
              (Opp to Anusha Vidhyalaya matriculation school),
              <br />
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-3 bg-slate-900 hover:bg-slate-800 text-white px-6 py-4 rounded-xl border border-slate-800 transition-colors font-bold group"
            >
              <i className="fa-solid fa-map-location-dot text-marlion-primary group-hover:text-white transition-colors"></i>{" "}
              Open in Google Maps
            </a>
          </div>
        </div>
