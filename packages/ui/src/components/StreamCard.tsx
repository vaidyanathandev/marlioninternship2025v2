...
export const StreamCard: React.FC<StreamCardProps> = ({
  ...
  return (
    <div
      ...
      ${
        isOpen
          ? "border-marlion-primary shadow-[0_0_30px_rgba(59,130,246,0.15)]"
          : "hover:border-marlion-primary/50"
      }`}
    >
      <div
      ...
      >
        <div className="flex items-center gap-5">
          <div
            className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300
              ${
                isOpen
                  ? "bg-gradient-to-br from-marlion-primary to-marlion-accent text-white shadow-lg"
                  : "bg-marlion-bg border border-marlion-border text-marlion-muted group-hover:text-white group-hover:border-marlion-primary/30"
              }`}
          >
            <i className={`fa-solid ${icon} text-2xl`}></i>
          </div>
          <div>
            <h3 className="text-xl font-bold text-white group-hover:text-marlion-primary transition-colors">
              {stream}
            </h3>
            <p className="text-xs text-marlion-muted mt-1 opacity-60 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-marlion-success"></span> Open for
              Registration
            </p>
          </div>
        </div>
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
            isOpen
              ? "bg-marlion-primary/20 text-marlion-primary rotate-180"
              : "text-marlion-muted bg-marlion-bg border border-marlion-border group-hover:border-marlion-primary/50"
          }`}
        >
      ...
      <div className={`${isOpen ? "block" : "hidden"} px-6 pb-8 pt-0 animate-fade-in`}>
        ...
        {videoUrl ? (
          <div className="aspect-video rounded-xl overflow-hidden border border-marlion-border">
            <iframe
              src={videoUrl}
